"""
In oder to evaluate the performance of mpu9250 by applying allan deviation, it is better to have the same
...
class MPU9250:
    ...
    def bus_open(self, max_speed_hz=10000000):
        # open(bus, device) --> Connects the object to the specified SPI device
...
# -----------------------------------------------------------------------------------------------
    ...
    def WriteReg(self, reg_address, data):
        self.bus_open()
        tx = [reg_address, data]        # tx --> transmit
        # xfer2([values]) returns:[values] --> Perform SPI transaction. CS (chip select?) will be held active between blocks
        rx = self.bus.xfer2(tx)     # return type: list[int]
        self.bus.close()        # Disconnects the object from the interface
        return rx                   # rx --> receive
...
# -----------------------------------------------------------------------------------------------
    ...
    def initialize(self, sample_rate_div=0x00, low_pass_filter_gt=0x01, low_pass_filter_a=0x01):
        MPU_InitRegNum = 11
        MPU_Init_Data = [[0, 0]] * MPU_InitRegNum

        MPU_Init_Data = [
        [0x80, self.__MPUREG_PWR_MGMT_1],   # Reset Device
        [0x01, self.__MPUREG_PWR_MGMT_1],          # Clock Source  0x01 --> Auto selects the best available clock source
        [0x00, self.__MPUREG_PWR_MGMT_2],          # Enable Acc & Gyro
        [sample_rate_div, self.__MPUREG_SMPLRT_DIV],  # Set sample rate, register SMPLRT_DIV is only used for 1kHz internal sampling
        [low_pass_filter_gt, self.__MPUREG_CONFIG],   # Use DLPF set Gyroscope bandwidth, temperature bandwidth
        [0x18, self.__MPUREG_GYRO_CONFIG],         # +-2000dps
        [0x08, self.__MPUREG_ACCEL_CONFIG],        # +-4G
        [low_pass_filter_a, self.__MPUREG_ACCEL_CONFIG_2],      # Set Acc Data Rates, Enable Acc LPF
        [0x30, self.__MPUREG_INT_PIN_CFG],         # INT_ANYRD_2CLEAR = 1: Interrupt status is cleared if any read operation is performed
        ...
        [0x20, self.__MPUREG_USER_CTRL],           # Enable I2C Master mode
...
# -----------------------------------------------------------------------------------------------
    ...
    def byte_to_float_le(self, input_buffer):
        byte_array = array.array("B", input_buffer)
        signed_16_bit_int, = struct.unpack("<h", byte_array)        # h means 2 bytes integer, < means little-endian
        return float(signed_16_bit_int)
